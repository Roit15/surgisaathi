...
export const ORG_ALTERNATE_NAME = "SURGISAATHI";
export const ORG_LEGAL_NAME = "SURGISAATHI HEALTHCARE PRIVATE LIMITED";
...
export function medicalConditionSchema({ data, path }) {
  if (!data.medicalCondition) return null;
  const { name, alternateName, associatedAnatomy, riskFactor, typicalTest } = data.medicalCondition;
  
  return {
    "@context": "https://schema.org",
    "@type": "MedicalCondition",
    "@id": `${SITE_URL}${path}#condition`,
    name: name,
    alternateName: alternateName,
    associatedAnatomy: associatedAnatomy,
    possibleTreatment: { "@id": `${SITE_URL}${path}#procedure` },
    riskFactor: riskFactor ? riskFactor.map(r => ({ "@type": "MedicalRiskFactor", name: r })) : undefined,
    typicalTest: typicalTest ? typicalTest.map(t => ({ "@type": "MedicalTest", name: t })) : undefined,
  };
}
